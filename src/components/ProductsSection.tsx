...
const ProductsSection = () => {
  ...
  const categories: Category[] = [{
    ...
    id: "protetor-matelasse",
    name: "PROTETOR DE MATELASSÊ",
  ...
  return <section id="produtos" className="bg-white py-16">
      <div className="section-container">
        <AnimatedSection>
          <h2 className="section-title">Nossos Produtos</h2>
          <p className="section-subtitle">Qualidade, variedade e proteção para o bem-estar de pessoas alérgicas há mais de 24 anos</p>
        </AnimatedSection>
        ...
        <div className="mb-8">
          <Tabs defaultValue="all" onValueChange={setActiveCategory} className="w-full">
            <TabsList className="hidden md:flex flex-wrap justify-center w-full h-auto bg-transparent gap-2 mb-4">
              {categories.map(category => <TabsTrigger key={category.id} value={category.id} className="flex items-center gap-1.5 px-4 py-2 data-[state=active]:bg-primary data-[state=active]:text-accent">
                  {category.icon}
            ...
            <div className="md:hidden overflow-x-auto pb-4">
              <div className="flex space-x-2 min-w-max">
                {categories.map(category => <button key={category.id} onClick={() => setActiveCategory(category.id)} className={`flex items-center gap-1.5 px-4 py-2 rounded-md whitespace-nowrap text-xs font-medium transition-colors
                      ${activeCategory === category.id ? 'bg-primary text-accent' : 'bg-gray-100 text-gray-800 hover:bg-gray-200'}`}>
                    {category.icon}
            ...
            <div className="md:hidden mt-4">
              ...
              loop: true
                ...
                <CarouselContent>
                  {filteredProducts.map(product => <CarouselItem key={product.id} className="md:basis-1/2 lg:basis-1/3">
                      <div className="p-1">
                        <div className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 h-full flex flex-col" onClick={() => openImageModal(product.image)}>
            ...
            <TabsContent value={activeCategory} className="mt-0">
              <AnimatedSection className="hidden md:grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                {filteredProducts.map(product => <div key={product.id} className="bg-white rounded-xl overflow-hidden shadow-md hover:shadow-lg transition-all duration-300 cursor-pointer h-full flex flex-col" onClick={() => openImageModal(product.image)}>
                    <div className="relative overflow-hidden aspect-square">
                      <img src={product.image} alt={product.name} className="w-full h-full object-cover transition-transform duration-300 hover:scale-110" />
                      <div className="absolute inset-0 bg-gradient-to-t from-black/60 to-transparent opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-end justify-center">
                        <div className="p-4 text-white text-center w-full">
        ...
        {expandedImage && <div className="fixed inset-0 z-50 bg-black/80 flex items-center justify-center p-4" onClick={closeImageModal}>
            <div className="relative max-w-4xl max-h-[90vh] w-full" onClick={e => e.stopPropagation()}>
              <button onClick={closeImageModal} className="absolute -top-10 right-0 text-white hover:text-primary p-2 z-10">
                Fechar
              </button>
              
              {/* Navigation arrows */}
              {filteredProducts.length > 1 && <>
                  <button onClick={showPreviousImage} className="absolute left-0 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/60 p-2 rounded-r-lg text-white z-10" aria-label="Imagem anterior">
                    <ChevronLeft className="h-8 w-8" />
                  </button>
                  
                  <button onClick={showNextImage} className="absolute right-0 top-1/2 -translate-y-1/2 bg-black/30 hover:bg-black/60 p-2 rounded-l-lg text-white z-10" aria-label="Próxima imagem">
                    <ChevronRight className="h-8 w-8" />
                  </button>
                </>}
              
              <div className="w-full h-full flex items-center justify-center">
                <img src={expandedImage} alt={filteredProducts[expandedIndex]?.name || "Imagem ampliada"} className="w-full h-auto max-h-[90vh] object-contain rounded-lg" />
              </div>
              
              {/* Product information */}
              {filteredProducts[expandedIndex] && <div className="absolute bottom-0 left-0 right-0 bg-black/60 p-4 text-white rounded-b-lg">
                  <h3 className="font-semibold text-lg">{filteredProducts[expandedIndex].name}</h3>
                  {filteredProducts[expandedIndex].description && <p className="text-sm text-gray-200">{filteredProducts[expandedIndex].description}</p>}
                </div>}
